  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {PAGES.map(({ key, Icon, route }) => (
        <div
          key={key}
          className="rounded-xl border border-sky-100 bg-white p-5 transition hover:border-sky-200 hover:shadow-sm"
        >
          <div className="mb-3 flex items-center gap-2">
            <div className="flex h-9 w-9 flex-none items-center justify-center rounded-lg bg-sky-50">
              <Icon size={18} className="text-sky-600" />
            </div>
            <code className="rounded bg-neutral-100 px-2 py-0.5 text-xs text-neutral-500">{route}</code>
          </div>
